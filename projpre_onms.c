/***************************************************************************************************************** 
File name: projpre_onms.c
Descrption: This program simulates the behaviour of a prefetcher that prefetches only when a cache miss ocuurs
******************************************************************************************************************/
...
uint64_t mask = SET_MASK;
	uint64_t identify_set = (next_line >> SET_OFFSET) & mask;
	uint64_t tag = next_line >> SET_TAG_SHIFT;
	writes++;
write_to_eight_way_on_a_miss(cache, identify_set, tag);
}
