/***************************************************************************************************************** 
File name: projpre_stride.c
Descrption: This program simulates the behaviour of a stride prefetcher
...
//direct mapped shared cache parameters
#define BLOCKS_IN_CACHE 8192//Enter the blocks in is_it_direct mapped cache(total cache size/block size)
#define DIR_INDEX 13//Enter the index bits required to address these blocks(2^(index bits) = no of blocks in cache)
#define DIR_MASK 0x1fff//Enter the mask to find Index bits(mask of 1's of length of index bits)
#define DIR_BLOCK_SIZE 8//Enter block size(in bytes)
#define DIR_OFFSET 3//Enter offset bits(2^(offset) = block size)
#define DIR_TAG_SHIFT 16//Enter tag shift required to find tag(DIR_INDEX+DIR_OFFSET)

//Eight way shared cache parameters
#define SETS_IN_EIGHT_WAY 1024//Enter sets in a 4 way cache(total cache size/block size = no of blocks, sets = no of blocks/4)
#define SET_INDEX 10//Enter the index bits required to address these sets(2^(index bits) = sets in cache)
#define SET_MASK 0x3ff//Enter the mask to find Index bits(mask of 1's of length of index bits)
#define SET_BLOCK_SIZE 8//Enter block size(in bytes)
#define SET_OFFSET 3//Enter offset bits(2^(offset) = block size)
...
//Fully asscociative shared cache parameters
#define FULL_BLOCKS_IN_CACHE 8192//Enter the blocks in is_it_direct mapped cache(total cache size/block size)
#define SETS_IN_FULL_CACHE 1//Enter sets in a 4 way cache
#define FULL_BLOCK_SIZE 8//Enter block size(in bytes)
#define FULL_OFFSET 3//Enter offset bits(2^(offset) = block size)
#define FULL_TAG_SHIFT 3//Enter tag shift required to find tag(SET_INDEX+SET_OFFSET)


//prefetch spec (4KB/8bytes)
...
struct pre_buffer{ 
	uint64_t blocks[PRE_BLOCKS];
	uint64_t no_of_access[PRE_BLOCKS];
	int no_of_hits;
	int no_of_misses; 
};
...
void stride_prefetch(struct pre_buffer *buffer, uint64_t address_in_trace);
void write_to_pbuf(struct pre_buffer *buffer, uint64_t tag);
void read_pre_buffer(struct pre_buffer *buffer, uint64_t address_in_trace);
			...
			else
			{
				...
				if(strncmp(argv[1], is_it_eightway, 24)==0){
					...
					in_pre_buffer = 0;
						if(j == 0)
						{
							stride[j] = abs(prev_address - address);
/*
printf("%x\n", j);							
printf("%x\n", prev_address);
printf("%x\n", address);
...
printf("%x\n", address);
printf("%d\n", stride[j]);*/
								stride_prefetch(&pbuf, address + stride[j]);
								prev_address = address;	
								j = 0;
							}
							else
	...
	if(strncmp(argv[1], is_it_eightway, 24)==0){
		...
		printf("No of cache Hits: %d\n", assoccache.no_of_hits+pbuf.no_of_hits);
		printf("No of cache Misses: %d\n", assoccache.no_of_misses);

		printf("Cache hit rate: %2.4f\n", (double)(assoccache.no_of_hits+pbuf.no_of_hits)/(assoccache.no_of_hits+pbuf.no_of_hits+assoccache.no_of_misses));
		printf("Cache miss rate: %2.4f\n", (double)assoccache.no_of_misses/(assoccache.no_of_hits+pbuf.no_of_hits+pbuf.no_of_hits+assoccache.no_of_misses));
...
void stride_prefetch(struct pre_buffer *buffer, uint64_t address_in_trace)
{
	...
	uint64_t tag = address_in_trace >> FULL_OFFSET;
	write_to_pbuf(buffer, tag);
}

void write_to_pbuf(struct pre_buffer *buffer, uint64_t tag)
{
	int least_recently_used = buffer->no_of_access[0];
	for(int x = 0; x < PRE_BLOCKS; x++){
		if(buffer->no_of_access[x] < least_recently_used){
			least_recently_used = buffer->no_of_access[x];
		}
	}
...
void read_pre_buffer(struct pre_buffer *buffer, uint64_t address_in_trace)
{
	...
	for(int i = 0; i < PRE_BLOCKS; i++){
		if(buffer->blocks[i] == tag){
			in_pre_buffer = 1;
			buffer->no_of_hits++;
buffer->no_of_access[i]++;
