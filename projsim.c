...
Functionalities:
1. Supports eight way associative and fully associative
2. Supports block size 64 KB
3. Supports LRU policy
4. Supports shared cache
How to run this program: First of all chosse between shared/split and eight way/fully associative. For example if a 
split 8-way cache behaviour to be simulated then edit the #defined parameters under the Eight way split cache parameters 
section.The parameters to be entered are sets in instruction eight way cache, sets in data eight way cache, number of index
...
cache type can be: eightway_cache or fully
trace file: file format should be plain text
For example an eight way cache command will look like: ./simulator eightway_cache trace
